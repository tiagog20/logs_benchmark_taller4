...
def main(input: str) -> Result:
    """Read JSON logs from S3 using Pandas and compute HTTP status rates."""
    ...
    for page in paginator.paginate(Bucket=bucket, Prefix=prefix):
        for obj in page.get("Contents", []):
            ...
            lines = [json.loads(line) for line in io.StringIO(body)]
            df = pd.DataFrame(lines)

            # Extract status codes
            df["status"] = (
                df["message"].str.extract(r"HTTP Status Code: (\d{3})").astype(float)
            )

            total += len(df)
            count_2xx += ((df["status"] >= 200) & (df["status"] < 300)).sum()
            count_4xx += ((df["status"] >= 400) & (df["status"] < 500)).sum()
            count_5xx += ((df["status"] >= 500) & (df["status"] < 600)).sum()
